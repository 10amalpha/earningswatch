...
// Manual date overrides - verified Feb 2, 2026
...
const DATE_OVERRIDES: Record<string, { date: string; time: 'bmo' | 'amc' | 'dmh' }> = {
        ...
        'MSTR': { date: '2026-02-05', time: 'amc' },  // Corrected to Feb 5
};
  ...
  for (const item of WATCHLIST) {
        ...
        // Sort earnings by date (ascending) and find the first future date
          ...
          results.push({
                    ...
                    ir: item.ir,
                    emoji: item.emoji,
                    date: DATE_OVERRIDES[item.ticker]?.date || nextEarning?.date || null,
                    time: (DATE_OVERRIDES[item.ticker]?.time || nextEarning?.hour as 'bmo' | 'amc' | 'dmh') || null,
